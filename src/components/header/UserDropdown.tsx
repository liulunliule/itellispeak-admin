...
export default function UserDropdown() {
  ...
  const fetchUserProfile = useCallback(async () => {
    try {
      ...
      if (response.data.code === 200) {
        setUserProfile({
          userName: response.data.data.userName || "Người dùng",
  ...
  const handleLogout = async () => {
    try {
      ...
      // Ignore error, still clear tokens
    }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="block mr-1 font-medium text-theme-sm">{userProfile.userName}</span>
        <svg
